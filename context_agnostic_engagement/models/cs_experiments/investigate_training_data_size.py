...
from context_agnostic_engagement.helper_tools.io_utils import load_lecture_dataset, get_fold_from_dataset, \
    get_label_from_dataset, get_features_from_dataset, get_cs_indexes
...
def main(args):
    ...
    for frac in [.1, .2, .3, .4, .5, .6, .7, .8, .9, 1.]:
        # get last fold as testing
        fold_train_df, fold_test_df = get_fold_from_dataset(lectures, 5, sample=frac)

        train_cs_idx, test_cs_idx, train_ncs_idx, test_ncs_idx = get_cs_indexes(fold_train_df, fold_test_df)
        ...
        joblib.dump(grid_model, join(args["output_dir"], "model_{}.pkl".format(frac)), compress=True)
        ...
        train_cs_rmse, test_cs_rmse = get_rmse(Y_train[train_cs_idx], Y_test[test_cs_idx],
                                               train_pred[train_cs_idx], test_pred[test_cs_idx])
        train_cs_spearman, test_cs_spearman = get_spearman_r(Y_train[train_cs_idx], Y_test[test_cs_idx],
                                                             train_pred[train_cs_idx], test_pred[test_cs_idx])

        train_ncs_rmse, test_ncs_rmse = get_rmse(Y_train[train_ncs_idx], Y_test[test_ncs_idx],
                                                 train_pred[train_ncs_idx], test_pred[test_ncs_idx])
        train_ncs_spearman, test_ncs_spearman = get_spearman_r(Y_train[train_ncs_idx], Y_test[test_ncs_idx],
                                                               train_pred[train_ncs_idx], test_pred[test_ncs_idx])
        ...
        best_model["train_cs_rmse"] = train_cs_rmse
        best_model["test_cs_rmse"] = test_cs_rmse
        best_model["train_cs_spearman_r"] = train_cs_spearman.correlation
        best_model["test_cs_spearman_r"] = test_cs_spearman.correlation
        # best_model["train_spearman_p"] = train_spearman.pvalue
        # best_model["test_spearman_p"] = test_spearman.pvalue

        best_model["train_ncs_rmse"] = train_ncs_rmse
        best_model["test_ncs_rmse"] = test_ncs_rmse
        best_model["train_ncs_spearman_r"] = train_ncs_spearman.correlation
        best_model["test_ncs_spearman_r"] = test_ncs_spearman.correlation
        # best_model["train_spearman_p"] = train_spearman.pvalue
        # best_model["test_spearman_p"] = test_spearman.pvalue

        best_model["fraction"] = frac
        #
